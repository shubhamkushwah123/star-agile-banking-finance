pipeline {
    ...
    stages {
       ...
       stage("Docker build"){
            steps {
				sh 'docker version'
				sh "docker build -t rakesh/bankapp-eta-app:${BUILD_NUMBER} ."
				sh 'docker image list'
				sh "docker tag rakesh/bankapp-eta-app:${BUILD_NUMBER} rakesh/bankapp-eta-app:latest"
            }
        }
		...
		stage('Push2DockerHub') {
			...
			steps {
				sh "docker push rakesh/bankapp-eta-app:latest"
			}
		}
